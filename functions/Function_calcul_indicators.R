...
###############################################################################
# 1. Calcul et sauvegarde de l’IRG (dPPI/dt) brute ----------------------------
###############################################################################
IRG_processing <- function(
    YEAR,
    DOY_range,
  ...
  # b) NDVImin / NDVImax : normaliser si valeurs brutes codées 0–10000/30000
  rng_max <- as.numeric(global(MAXV, "max", na.rm = TRUE)[[1]])
  if (!is.na(rng_max) && is.finite(rng_max) && rng_max > 3.5) {
    MINV <- MINV / 10000
    MAXV <- MAXV / 10000
    message("→ MINV et MAXV normalisés (/10000) car valeurs brutes détectées (max > 3.5).")
  }
  
  ## 3. Affichage des plages de MINV/MAXV pour diagnostic
  rng_MINV <- global(MINV, fun = c("min","max"), na.rm = TRUE)
  rng_MAXV <- global(MAXV, fun = c("min","max"), na.rm = TRUE)
  cat(sprintf(
    "MINV: min=%.4f, max=%.4f\n",
    rng_MINV[[1]], rng_MINV[[2]]
  ))
  cat(sprintf(
    "MAXV: min=%.4f, max=%.4f\n",
    rng_MAXV[[1]], rng_MAXV[[2]]
  ))
  
  ## 4. Dérivée analytique de la double logistique *avec* AMPL = MAXV–MINV
  DLd.f <- function(DOY, MINV, MAXV, ONSET, OFFSET, GROWTH, SENESC) {
    AMPL <- MAXV - MINV
    AMPL * (
      GROWTH * exp(-GROWTH * (DOY - ONSET)) /
        (1 + exp(-GROWTH * (DOY - ONSET)))^2
      -
        SENESC * exp(-SENESC * (DOY - OFFSET)) /
        (1 + exp(-SENESC * (DOY - OFFSET)))^2
    )
  }
  
  ## 5. Empiler IRG pour chaque DOY
  irg_list <- lapply(DOY_range, function(doy) {
    lapp(
      c(MINV, MAXV, ONSET, OFFSET, GROWTH, SENESC),
      fun      = DLd.f,
      filename = "",
      DOY      = doy
    )
  ...
  ## 6. Sauvegarde de l’IRG saisonnier
  out_tif <- file.path(
    IRG_data_case,
    paste0("IRG_season_", site, "_", YEAR, ".tif")
  )
  writeRaster(irg_stack, out_tif, overwrite = TRUE)
  message("▶ IRG saisonnier (dPPI/dt) empilé et sauvé sous :\n", out_tif)
}
...
###############################################################################
# IRG_processing : calcule et empile dPPI/dt pour une plage de jours DOY
###############################################################################
IRG_processing <- function(
    YEAR,
    DOY_range,
  ...
  ## 1. Chargement des rasters PARAMÈTRES déjà normalisés --------------------
  params <- c("MINV", "MAXV", "ONSET", "OFFSET", "GROWTH", "SENESC")
  rasters <- setNames(
    lapply(params, function(p) {
      fp <- file.path(input_brut_data_case,
  ...
  ## 2. Contrôles rapides (aucune correction automatique) --------------------
  # a) plages PPI prévues 0–3 (≈3,5 max dans des cas extrêmes)
  if (global(MAXV, "max", na.rm = TRUE)[[1]] > 3.5)
    warning("MAXV contient des valeurs > 3,5 : vérifiez l’échelle.")
  
  # b) ONSET / OFFSET doivent être entre 1 et 365
  out_doy <- which(ONSET[] < 1 | ONSET[] > 365 |
                     OFFSET[] < 1 | OFFSET[] > 365)
  if (length(out_doy))
    warning("Des ONSET / OFFSET hors [1‑365] détectés (non modifiés).")
  
  ## 3. Dérivée analytique de la double logistique ---------------------------
  DLd.f <- function(DOY, MINV, MAXV, ONSET, OFFSET, GROWTH, SENESC) {
    AMPL <- MAXV - MINV
    AMPL * (
      GROWTH  * exp(-GROWTH  * (DOY - ONSET))  / (1 + exp(-GROWTH  * (DOY - ONSET)))^2 -
