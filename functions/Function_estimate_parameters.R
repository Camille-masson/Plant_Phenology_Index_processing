...
estimate_parameters <- function(VAR, mask_shp, resolution, downloads_case, YEAR, brut_data_case) {
  ...
  # On encapsule l'appel à vect() dans try(), pour ne pas laisser l'erreur sortir du bloc
  mask_vec <- try(vect(mask_shp), silent = TRUE)  # doit être en EPSG:2154
  
  # Si vect() a levé une erreur, ou si l'objet résultant est vide, on déclenche le warning et on passe à la suite
